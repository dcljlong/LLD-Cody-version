...
class ProjectCreate(BaseModel):
    ...
    programme_start_date: Optional[str] = None
    required_finish_date: Optional[str] = None
...
class ProgrammeTaskResponse(BaseModel):
    ...
    original_programme_start_date: Optional[str] = None
...
class TaskCreate(BaseModel):
    ...
    linked_programme_task_id: Optional[str] = None
...
class TaskUpdate(BaseModel):
    ...
    linked_task_codes: Optional[List[str]] = None
    planned_start: Optional[str] = None
    planned_finish: Optional[str] = None
...
class ScopeTaskLinkUpdate(BaseModel):
    linked_programme_task_id: Optional[str] = None
...
async def create_project(project: ProjectCreate, current_user: dict = Depends(get_current_user)):
    ...
    doc = {
        ...
        **project.model_dump(),
        "created_at": now,
        "updated_at": now,
        "display_order": project.display_order if project.display_order is not None else 9999
    }
...
async def get_projects(current_user: dict = Depends(get_current_user)):
    projects = await db.projects.find({"user_id": current_user["id"]}, {"_id": 0}).sort("display_order", 1).to_list(1000)
...
async def update_project(project_id: str, project: ProjectCreate, current_user: dict = Depends(get_current_user)):
    ...
    result = await db.projects.update_one(
        ...
        {"$set": {**project.model_dump(), "updated_at": now, "display_order": project.display_order if project.display_order is not None else 9999}}
    )
...
async def delete_project(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    await db.gates.delete_many({"project_id": project_id, "user_id": current_user["id"]})
    await db.action_items.delete_many({"project_id": project_id, "user_id": current_user["id"]})
    await db.walkaround_entries.delete_many({"project_id": project_id, "user_id": current_user["id"]})
...
async def delete_gate(gate_id: str, current_user: dict = Depends(get_current_user)):
    """Delete a gate owned by the current user"""
    existing = await db.gates.find_one({"id": gate_id, "user_id": current_user["id"]})
    if not existing:
        raise HTTPException(status_code=404, detail="Gate not found")

    result = await db.gates.delete_one({"id": gate_id, "user_id": current_user["id"]})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Gate not found")

    now = datetime.utcnow().isoformat()
    await db.walkaround_entries.update_many(
        {"gate_id": gate_id, "user_id": current_user["id"]},
        {"$set": {"gate_id": None, "updated_at": now}}
    )
...
async def create_default_gates(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    default_gates = [
        ...
        {"name": "Linings Complete", "owner_party": "YOU", "days_offset": 35, "is_hard_gate": False, "is_optional": False, "depends_on": [2]},
...
async def get_walkaround_entries(
    ...
    if project_id:
        query["project_id"] = project_id
    if date:
        # Filter by NZ local date (entries created on that local day)
        from zoneinfo import ZoneInfo
        nz_tz = ZoneInfo("Pacific/Auckland")
        local_start_dt = datetime.fromisoformat(f"{date}T00:00:00").replace(tzinfo=nz_tz)
        local_end_dt = local_start_dt.replace(hour=23, minute=59, second=59, microsecond=0)
...
async def get_project_diary(
    ...
    if date:
        target_date = date
        local_start_dt = datetime.fromisoformat(f"{target_date}T00:00:00").replace(tzinfo=nz_tz)
    else:
        local_start_dt = datetime.now(nz_tz).replace(hour=0, minute=0, second=0, microsecond=0)
        target_date = local_start_dt.date().isoformat()
    ...
    if task_ids:
        ...
        task_lookup = {
            t["id"]: {
                ...
                "status": t.get("status"),
                "owner_party": t.get("owner_party"),
                "linked_programme_task_id": t.get("linked_programme_task_id")
            }
            for t in tasks
        }
...
async def upload_programme(data: ProgrammeUpload, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        with open(tmp_path, "rb") as pdf_handle:
            current_openai_client = OpenAI(api_key=api_key)
            uploaded_file = current_openai_client.files.create(
                file=pdf_handle,
                purpose="user_data"
            )
            ...
            input=[
                {
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": """You are a construction programme parser. Extract all tasks from the construction programme PDF.
For each task, extract:
- task_name: The name/description of the task
- programme_start_date: Start date in YYYY-MM-DD format (if visible)
...
Example: [{"task_name": "Site Setup", "programme_start_date": "2024-01-15", "end_date": "2024-01-20", "duration_days": 5, "dependencies": [], "owner": "MC"}]"""
                        }
                    ]
                },
                {
        ...
        for task in tasks_data:
            ...
            task_doc = {
                ...
                "programme_start_date": task.get("programme_start_date"),
                "end_date": task.get("end_date"),
                "duration_days": task.get("duration_days"),
                "dependencies": task.get("dependencies", []),
                "owner_tag": "UNASSIGNED",
                "is_tracked": False,
                "original_programme_start_date": task.get("programme_start_date"),
...
async def update_task_dates(task_id: str, data: TaskDateUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if data.delay_reason:
        change_log = {
            ...
            "user_id": current_user["id"],
            "original_start": task.get("programme_start_date"),
            "original_end": task.get("end_date"),
            "new_start": data.programme_start_date,
...
async def get_tasks(job_id: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    query = {}
    if job_id:
        query["job_id"] = job_id
...
async def create_task(data: TaskCreate, current_user: dict = Depends(get_current_user)):
    ...
    task_doc = {
        ...
        "source_programme_id": data.source_programme_id,
        "linked_programme_task_id": data.linked_programme_task_id,
        "task_type": data.task_type,
        "linked_task_codes": data.linked_task_codes,
...
@api_router.put("/tasks/{task_id}/link-programme-task", response_model=TaskResponse)
async def link_scope_task_to_programme_task(task_id: str, data: ScopeTaskLinkUpdate, current_user: dict = Depends(get_current_user)):
    task = await db.tasks.find_one({"id": task_id}, {"_id": 0})
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    if data.linked_programme_task_id:
        programme_task = await db.tasks.find_one({"id": data.linked_programme_task_id, "job_id": task["job_id"], "task_type": {"$ne": "Scope"}}, {"_id": 0})
        if not programme_task:
            raise HTTPException(status_code=404, detail="Programme task not found")
    await db.tasks.update_one(
        {"id": task_id},
        {"$set": {"linked_programme_task_id": data.linked_programme_task_id}}
    )
    updated = await db.tasks.find_one({"id": task_id}, {"_id": 0})
    return TaskResponse(**updated)
            ...
            input=[
                {
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": """You are an expert NZ commercial interior fitout project manager.
            ...
            for sugg in ai_suggestions:
                ...
                if matching_task:
                    suggestions.append({
                        ...
                        "programme_start_date": matching_task.get("programme_start_date"),
    ...
    templates = [
        {
            ...
            "gates": [
                ...
                {"name": "Partitions Complete", "owner_party": "YOU", "typical_duration_days": 14, "buffer_days": 2, "depends_on_order": [1], "is_hard_gate": False, "is_optional": True, "order": 2},
                ...
                {"name": "Linings Complete", "owner_party": "YOU", "typical_duration_days": 14, "buffer_days": 2, "depends_on_order": [3], "is_hard_gate": False, "is_optional": False, "order": 4},
                ...
                {"name": "Subbies 2nd Fix to Ceiling Grid", "owner_party": "SUBBIES", "typical_duration_days": 7, "buffer_days": 2, "depends_on_order": [6], "is_hard_gate": False, "is_optional": False, "order": 7},
            ],
            "created_at": now
        },
        {
...
async def get_jobs():
    ...
    return {"value": jobs, "Count": len(jobs)}
...
async def get_job_analysis(job_id: str):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    return job.get("latest_analysis") or {}

@api_router.get("/jobs/{job_id}/programme")
async def get_job_programme(job_id: str):
    job = await db.jobs.find_one({"id": job_id}, {"_id": 0})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    latest_analysis = job.get("latest_analysis") or {}
    return latest_analysis.get("proposed_programme") or []
