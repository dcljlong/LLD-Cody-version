...
const ActionItemsPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const actionItemsOnly = loadedItems.filter(i => !i.source_programme_id);
      setItems(actionItemsOnly);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedItem) {
        await actionItemsApi.update(selectedItem.id, formData);
        toast.success('Item updated');
      } else {
        await actionItemsApi.create(formData);
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-muted-foreground">
                {getProjectName(getItemProjectId(item))}
              </span>
              {getDueDate(item) && (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-muted-foreground">
              ...
              {item.expected_complete_date && (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-muted-foreground">
                  ...
                  Expected {new Date(item.expected_complete_date).toLocaleDateString('en-NZ', { day: '2-digit', month: 'short' })}
                </span>
              )}
              {item.owner && (
                <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-muted-foreground">
              ...
              {(item.created_by_name || item.user_name || item.creator_name) && (
                <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-muted-foreground">
                  By {item.created_by_name || item.user_name || item.creator_name}
                </span>
              )}
            </div>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-9 w-9 rounded-xl border border-white/10 bg-white/[0.03] opacity-80 transition-all hover:bg-white/[0.08] hover:opacity-100"
                >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                {getProjectName(getItemProjectId(item))}
              </span>
