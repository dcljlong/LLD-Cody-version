      ...
      {diary && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <Card className="ops-card">
              <CardContent className="pt-3 pb-3 text-center">
                <p className="text-2xl font-heading font-bold text-red-500">{diary.summary.blocked_gates}</p>
                <p className="text-xs text-muted-foreground uppercase">Blocked</p>
              </CardContent>
            </Card>
            <Card className="ops-card">
              <CardContent className="pt-3 pb-3 text-center">
                <p className="text-2xl font-heading font-bold text-amber-500">{diary.summary.at_risk_gates}</p>
                <p className="text-xs text-muted-foreground uppercase">At Risk</p>
              </CardContent>
            </Card>
            <Card className="ops-card">
              <CardContent className="pt-3 pb-3 text-center">
                <p className="text-2xl font-heading font-bold text-red-400">{diary.summary.overdue_items}</p>
                ...
                {diary.walkaround_entries?.length > 0 ? (
                  <div className="space-y-3">
                    {diary.walkaround_entries.map(entry => (
                      <div key={entry.id} className="p-3 bg-secondary/30 rounded-sm">
                        <p className="text-sm">{entry.note}</p>
                        <p className="text-xs text-muted-foreground mt-2">
                          {new Date(entry.created_at).toLocaleTimeString('en-NZ', { hour: '2-digit', minute: '2-digit' })}
                    ...
                    {diary.blocked_gates.map(gate => (
                      <div key={gate.id} className="p-2 bg-red-950/30 rounded-sm border-l-4 border-l-red-500">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Owner: {gate.owner_party} • Required: {new Date(gate.required_by_date).toLocaleDateString('en-NZ', { day: '2-digit', month: 'short' })}
                        </p>
                      </div>
                    ))}
                  </div>
